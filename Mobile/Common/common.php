...
function sendmail($tomail,$subject,$body,$config=''){
		...
		//$mail->AddReplyTo('287804931@qq.com', 'Mobile'); //回复到这个邮箱
...
function template_file($module='',$path='',$ext='html'){
	...
	$path= $path ? $path : TMPL_PATH.'Wap/'.$sysConfig['DEFAULT_THEME'].'/';//手机版
...
//查询分类某个属性值
function getcatvar($table,$where,$field,$k=1){
	$res = D($table)->field($field)->where($where)->find();
//        print_r($res);
        if($k){
            echo $res[$field];
        }else{
